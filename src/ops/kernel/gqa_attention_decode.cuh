...
__launch_bounds__(256) __global__ void gqa_attention_small_t_reduce_output_kernel(
    ...
    if (gate == nullptr) {
        ...
        // BASEOPT-6: fused sigmoid gate. Replicates the standalone elementwise kernel
        // bit-for-bit: bf16 rounding of the reduce result first, then the fp32 multiply
        // by sigmoid(gate), then round-to-nearest bf16 store.
