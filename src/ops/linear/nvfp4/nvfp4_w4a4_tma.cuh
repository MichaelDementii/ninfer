...
Nvfp4W4a4TmaDescriptors make_nvfp4_w4a4_tma_descriptors(const std::uint8_t* activation_codes,
                                                        ...
                                                        std::int32_t tokens) {
    // The quantizer writes the activation scale plane in tiles of kNvfp4TmaBlockM tokens, so a
    // schedule with any other BlockM would address those tiles mis-shaped. The two have to agree
    // and this is where the shape is encoded, so pin it here rather than rely on the schedules
    // that happen to be registered.
...
struct Nvfp4W4a4TmaSchedule {
    // The activation scale plane is written in tiles of this width; see nvfp4_config.h.
    static_assert(BlockM == kNvfp4TmaBlockM);
...
#pragma unroll
        for (int mma_n = 0; mma_n < Schedule::kMmaN; ++mma_n) {
            ...
            // The last M tile may be partial. The activation descriptors carry the token
            // count as their row extent, so TMA zero-fills the rows past the end and the
            // padded lanes only have to stay off other people's memory: clamp the token index
            // the epilogue reads with, and drop the store below.
