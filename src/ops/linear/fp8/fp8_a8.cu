...
// 256-token tile, a width that is a whole number of the cp.async route's 64-token tiles, and a
// modelled cost below that route's. A partial trailing tile is allowed within those bounds: TMA
// zero-fills the rows past the descriptor extent, and the epilogue neither reads their activation
// scale nor stores them. Everything else keeps the cp.async route.
