...
#pragma unroll
    ...
    for (int task = consumer_thread; task < kOutputVectors; task += Schedule::kConsumerThreads) {
        ...
        // The last M tile may be partial: the activation descriptors carry the token count as
        // their row extent, so TMA zero-fills the rows past the end and those lanes compute
        // values that simply must not be stored.
