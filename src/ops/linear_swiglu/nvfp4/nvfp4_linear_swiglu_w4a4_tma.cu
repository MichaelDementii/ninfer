...
Nvfp4W4a4TmaDescriptors make_descriptors(const std::uint8_t* activation_codes,
    ...
    // Same agreement as the shared TMA route: the quantizer writes the scale plane in tiles of
    // kNvfp4TmaBlockM tokens, so this schedule has to address them at that width.
    static_assert(Schedule::kBlockM == kNvfp4TmaBlockM);
    constexpr std::uint32_t kCodeColumns = 64;
    // Activation scales arrive tile-contiguous, one [kNvfp4TmaBlockM tokens,
    // kNvfp4ScaleTileGroups groups] tile per request.
