...
#pragma unroll
        for (int m = 0; m < MM; ++m) {
            LDM4(af[m][0], af[m][1], af[m][2], af[m][3],
                 smem_u32(&buf[((lane + m * 16 + it) & (SMEM_ROWS - 1)) * ROW_BYTES]));
            sa[m] = reinterpret_cast<unsigned*>(buf)[((lane & 31) * 4 + (m & 3)) & 255];
        }
#pragma unroll
        for (int n = 0; n < NN; ++n) {
            LDM2(bf[n][0], bf[n][1],
                 smem_u32(&buf[((lane + n * 8 + it + 33) & (SMEM_ROWS - 1)) * ROW_BYTES]));
            sb[n] = reinterpret_cast<unsigned*>(buf)[(((lane >> 2) & 31) * 4 + (n & 3)) & 255];
        }
#pragma unroll
        for (int m = 0; m < MM; ++m) {
#pragma unroll
...
int main() {
    ...
    run("fp8 4x8", fp8_tile, 32.0, sink);
    run("nvfp4 4x8 конфликтные масштабы", nvfp4_tile, 64.0, sink);
    return 0;
}
