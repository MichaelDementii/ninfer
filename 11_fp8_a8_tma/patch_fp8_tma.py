#!/usr/bin/env python3
"""П4: fp8-GEMM префилла на TMA с варп-специализацией.

Сейчас `fp8_mma_kernel` стейджит через `cp.async` и держит две стадии, потому что бюджет shared
(99 КиБ) при `Stages · (BlockTokens + BlockRows) · BlockK` не даёт большего тайла при двух CTA на
SM. Свип из 13 точек это подтвердил: всё, что опускает `MinBlocksPerSm` до 1, теряет 0.5–6.2%,
всё, что поднимает до 4 ценой мелкого K, теряет 3%. То есть 72–85% от потолка fp8 — свойство
структуры, а не настройки.

Соседний `nvfp4_w4a4_tma_kernel` устроен иначе: отдельные нити-продюсеры выдают
`cp.async.bulk.tensor` под `mbarrier`, консьюмеры ждут только свою стадию, и один CTA на SM с
большим тайлом обгоняет два CTA с маленьким. Он даёт 84–97% от своего потолка.

Здесь тот же приём применён к fp8. Отличие в пользу fp8: масштабы у него **построчные** (float на
токен, bf16 на строку весов), а не поблочные, поэтому дескрипторов нужно два, а не четыре, и
масштабы читаются прямо в эпилоге.

Тайл K взят 64 байта — ровно как у nvfp4 (там 128 значений по 4 бита). Значит раскладка в shared и
формула свизла `((logical_byte >> 4) ^ ((row >> 1) & 3)) * 16 + (logical_byte & 15)` переносятся
дословно, вместе с адресацией `ldmatrix`.
"""
import sys
...
Fp8A8TmaDescriptors make_fp8_a8_tma_descriptors(const std::uint8_t* activation_codes,
    ...
    descriptors.a_codes = fp8_make_tma_2d(const_cast<std::uint8_t*>(activation_codes),
                                          Geometry::kInputRows, static_cast<std::uint64_t>(tokens),
                                          Geometry::kInputRows, kRowBytes, BlockM,
                                          "encode fp8 activation codes TMA");
    descriptors.b_codes = fp8_make_tma_2d(const_cast<std::uint8_t*>(weight_codes),
                                          Geometry::kInputRows, Geometry::kOutputRows,
                                          Geometry::kInputRows, kRowBytes, BlockN,
                                          "encode fp8 weight codes TMA");
    return descriptors;
}
...
__global__ __launch_bounds__(Schedule::kThreads,
                             Schedule::kMinBlocksPerSm) void fp8_a8_tma_kernel(
...
#pragma unroll
            for (int mma_m = 0; mma_m < Schedule::kMmaM; ++mma_m) {
                ...
                const auto* address =
                    tensors.a_codes[stage] + row * Schedule::kRowBytes +
...
#pragma unroll
            for (int mma_n = 0; mma_n < Schedule::kMmaN; ++mma_n) {
                ...
                const auto* address =
                    tensors.b_codes[stage] + row * Schedule::kRowBytes +
...
#pragma unroll
        for (int mma_n = 0; mma_n < Schedule::kMmaN; ++mma_n) {
            ...
            const float value00            = epilogue.apply(
                parent_row0, token0, accumulators[mma_m][mma_n][0] * activation_scale0 * weight_scale.x);
            const float value01 = epilogue.apply(
                parent_row1, token0, accumulators[mma_m][mma_n][1] * activation_scale0 * weight_scale.y);
            const float value10 = epilogue.apply(
                parent_row0, token1, accumulators[mma_m][mma_n][2] * activation_scale1 * weight_scale.x);
            const float value11 = epilogue.apply(
                parent_row1, token1, accumulators[mma_m][mma_n][3] * activation_scale1 * weight_scale.y);
...
open(f'{R}/src/ops/linear/fp8/fp8_a8_tma.cuh', 'w', encoding='utf-8').write(HEADER)
print('создан src/ops/linear/fp8/fp8_a8_tma.cuh')

# --- диспетчер: маршрут TMA там, где число токенов кратно тайлу ---------------------------------
...
struct Fp8LinearA8TmaSchedule {
    using Type = Fp8A8TmaSchedule<256, 3, 1>;
};
...
print('диспетчер и расписание готовы')
