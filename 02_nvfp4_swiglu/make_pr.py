#!/usr/bin/env python3
"""Assemble and file the NVFP4 fused-SwiGLU registration submission, one explicit stage at a time.
...
BRANCH = "perf/nvfp4-fused-swiglu-every-width"
REMOTE_BRANCH = "perf/nvfp4-fused-swiglu-every-width"
BASE_BRANCH = "master"
...
EXPECT_COMMIT = "93959c1657eed5d926005d9bd4e5555f3c148504"
...
def stage_bundle(args):
    ...
    # leaving a stale /tmp/nv.bundle to be shipped.
    run(["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=30", "-i", key, "-p", PORT, SCP_HOST,
         f"rm -f /tmp/nv.bundle && cd {SERVER_REPO} && "
         f"git bundle create /tmp/nv.bundle origin/{BASE_BRANCH}..{BRANCH} --branches={BRANCH}"])
    BUNDLE.unlink(missing_ok=True)
    run(["scp", "-q", "-o", "BatchMode=yes", "-i", key, "-P", PORT,
         f"{SCP_HOST}:/tmp/nv.bundle", str(BUNDLE)])
