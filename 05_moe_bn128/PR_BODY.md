...
The routed grouped GEMMs run one job per expert column tile, and the tile is 64 columns wide at
every chunk width. Each job re-reads its expert's entire weight slab, and at top-8 of 256 the
average column run per expert is `tokens / 32`, so a 64-wide job covers a 2048-token slice exactly,
re-reads every slab twice at 4096 and four times at 8192. The weight traffic of the routed MoE is a
function of the job width and the slice width together, and only one of the two follows the caller.

**Process.** Opened as #ISSUE first and sent after your reply. Targeted at `master`, one commit on
`TBD_BASE`; every number here was measured at `1fc1cb76`. No public contract moves: the workspace
query, the arena sequence and every op signature are untouched. What moves is occupancy on the wide
tile, deliberately, and it is measured below.

## What changes

Two files. The kernels are already templated on `ExpertBN`; what was missing is a 128-wide
instantiation and a rule for choosing it.
...
fragments and its epilogue were written for exactly one eight-column group per warp, the way
`sparse_moe_prefill_q4_gate_up_kernel` already was not.

A 128-wide tile doubles `Bs` and three blocks per SM stop fitting, so one expression owns both the
launch bound and the grid:
...
There is no environment variable and no launch macro. Two earlier commits of ours did this through
`NINFER_MOE_BN` and two `#define`s; neither is here, because a hidden mutable channel in production
code is what you told us not to bundle.

## Occupancy is the price, and it is arithmetic

The device reports `sharedMemPerMultiprocessor` 102400 and `reservedSharedMemPerBlock` 1024.
`cuobjdump --dump-resource-usage` on the two binaries:
...
Registers hold two blocks and only just: 126 x 256 x 2 = 64512 against 65536, two to spare on the
gate/up. Nothing spills.

**The 19 existing `sparse_moe_prefill` instantiations are unchanged in registers, shared, local and
stack.** `prefill_blocks_per_sm(64)` is the 3 the literal used to be, and the `WarpNT`
generalisation collapses to the old shape at `ExpertBN == 64`, where `WarpNT` is 1.
...
Two controls, both by construction. The 1024 and 2048 rows sit below
`kSparseMoePrefillWideBnTokens`, so the plan cannot reach the wide tile there. And **`w8-w8` never
can**: the W8 routed codec goes through `sparse_moe_prefill_w8_gate_up_kernel` and
`sparse_moe_prefill_w8_down_kernel`, which are not the templates this widens. Eight cells, 0.9981 to
1.0015, and that is the floor the Q4 rows should be read against.

## Workspace

`workspace_bytes` is identical to master at every width and every codec: 43,378,176 / 86,752,768 /
173,501,952 / 173,501,952. The engine's `gpu workspace peak` is 104.38 MiB at chunk 1024 and
835.06 MiB at chunk 8192 on both arms. No allocation site, arena boundary or public query moves.
...
Inside the bucket the two widened GEMMs are the whole of it:

| kernel | master ms | branch ms |
| --- | --: | --: |
| `q4_gate_up<8, 64>` -> `<8, 128>` | 709.31 | **667.33** |
| `qx_down<Q5DownMma, 8, 64>` -> `<8, 128>` | 361.19 | **330.64** |
| `w8_gate_up<false, false>`, code unchanged | 95.48 | 92.12 |
...
The chunk-1024 row is the control the predicate guarantees, and it reads -0.02% over four rounds. I
quote the paired per-round ratios rather than a mean of means because the absolute rate drifts
...
- **I did not sweep the switch point.** `kSparseMoePrefillWideBnTokens` is 4096 because that is
  ...
  the wide tile winning at 4096 and 8192 and unreachable below. Explaining is not measuring: I did
  not lower the constant and measure 2048 and 3072 on both tile widths, which is the evidence that
  would justify the number.
- No SASS census. The resource table says the 19 existing instantiations keep their registers,
  shared, local and stack, and the greedy gate says the output is unchanged, but I did not compare
  instruction streams body by body.
...
- `docs/performance.md` publishes prefill throughput at a 1024-token chunk, which this change cannot
  reach, so I do not expect those figures to move; I have not re-run that harness and propose no
  edits to the document.
- Qwen3.8-27B NVFP4 takes no sparse-MoE prefill route and is not measured. Speculation is off, KV is
  bf16 in every end-to-end run here.
